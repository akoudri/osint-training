...
# Build filter_complex string dynamically with downscale to 1080p
FILTER=""
CONCAT_INPUTS=""
...
for i in "${!SEGMENTS[@]}"; do
    ...
    # Add scale filter to downscale from 4K to 1080p for faster encoding
    FILTER+="[0:v]trim=start=$START:end=$END,setpts=PTS-STARTPTS,scale=1920:1080:flags=bilinear[v$INDEX]; "
...
echo "Extracting $SEGMENT_COUNT segments from '$INPUT_FILE' to '$OUTPUT_FILE' (downscaled to 1080p)..."
